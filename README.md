...
Строки записываются в тег `<text>`: один `<init>` на строку плюс общий `<data>` с кодами символов строк. В `init` строки хранятся цвет, позиция, font_index, char_offset и char_count. Кернинг не задаётся вручную у строки: он берётся из пар, записанных внутри соответствующего `<font>`.
